...
export const api = {
  ...
  // Payload includes lat/lng/accuracy from navigator.geolocation. Server
  // hard-blocks with 403 when the fence is enabled and the reading is
  // missing or outside the radius — no override, employee must be inside.
  clockIn: (employee_id: string, geo?: { lat?: number; lng?: number; accuracy?: number }) =>
  ...
  updatePayrollConfig: (data: {
    ...
    working_days_convention: 'fixed_30' | 'actual_month' | 'actual_working_days';
    salary_mode: 'flat' | 'structured';
  }) => request<{ ok: true; saved: {
    basic_pct: number; hra_pct: number; special_allowance_pct: number; employer_pf_pct: number;
    working_days_convention: 'fixed_30' | 'actual_month' | 'actual_working_days';
    salary_mode: 'flat' | 'structured';
  } }>('/payroll/config', { method: 'PUT', body: JSON.stringify(data) }),
